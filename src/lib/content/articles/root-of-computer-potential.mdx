...
Such arbitrary assignment of numeric values to some <Note>_phenomenon_<span slot="note">: a fact or situation that is observed to exist or happen</span></Note> doesn't need to describe it completely, nor perfectly. It just needs to be useful for the purpose you're using it for.
